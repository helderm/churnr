...
\begin{otherlanguage}{swedish}
  \begin{abstract}
		Leverantörer av onlinetjänster har bevittnat en snabb tillväxt av deras användarbas under de senaste åren. Fenomenet har lockat ökande antal av konkurrenter som avsedd att uppnå sin egen andel av marknaden. I detta sammanhang har kostnaden för att locka nya kunder ökat avsevärt, vilket ökar vikten av att behålla befintliga kunder. Därför har blivit gradvis viktigare för företagen att förbättra användarupplevelsen och se till att de håller en större andel av sina användare aktiv i konsumtionen av sina produkter. Sådär är företagen tvungna att bygga verktyg som kan identifiera vad som ber kunderna att stanna och också identifiera användarnas avsikt att överge tjänsten. Fokus på i denna avhandling är för att belysa problemet med hur man kan förutsäga användaravbrott och upptäcka drivrutiner för användaruppehåll på en onlinetjänster.

Klassiska modeller som logistisk regression och slumpmässiga skogar har använts för att förutsäga kundens churn-sannolikhet med en ganska stor precision tidigare, vanligen genom att aggregera all känd information om en användare över en tidsperiod till en unik datapunkt. Å andra sidan har återkommande neurala nätverk, särskilt LSTM-varianten (Long Short Term Memory), visat ett imponerande resultat för andra domäner som taligenkänning och videoklassificering, där data behandlas som sekvens istället. Denna avhandling undersöker hur LSTM-modellerna utför för att förutse churn jämfört med vanliga icke-sekventiella baslinjemetoder när de tillämpas på användarbeteende data av en välkänd musikstreamingtjänst. Det undersöktes också hur olika aspekter av data, som fördelningen mellan klasserna för användaravbrott och behållning, storleken på användarhändelseshistorik och funktionrepresentation, påverkar prestandan av förutsägande modeller. De erhållna resultaten visar att LSTM har en jämförbar prestanda till slumpmässig skog för upptäckt av användaravbrott, samtidigt som den är signifikant bättre än logistisk regression. LSTMs visar sig således vara ett lämpligt val för att förutsäga churn på användarnivå. I tillägg är en ram ordnad för att skapa en dataset som är lämplig för träning av prediktiva modeller, vilket kan undersökas ytterligare för att analysera användarbeteende och att skapa kvarhållningsåtgärder som minimerar övergivna kunder.
  \end{abstract}
\end{otherlanguage}
...
\chapter{Introduction}
		...
		In a world where service providers face an ever increasing competition on the market, retaining the already acquired customers is rising in importance for any company that wants to stay in business. This is specially true for industries that are mainly web-based: startups nowadays can provide an online service with  an impressively low amount of initial capital, ingressing into a market with relative ease. While ousting a well-established company from the top is a hard feat to accomplish, it is far from being an unheard event, urging business to set up specialized tools to identify unsatisfied clients and plan retention actions. 
		
	The act of a user abandoning a service provider, either by discontinuation of a formal contract or inactivity, is called \emph{churn}. Customer relationship management tools  aims to minimize this metric so as to increase profitability and improve business health. For doing so, an important step that must be performed is to identify the users more likely to churn in the near future so as to know on which customers the retention actions should be focused. These actions may constitute a significant cost to the company when executed, so it is desirable that errors in classification are minimized. The task of detecting how likely a user is to abandon a service is called \emph{churn prediction}, and it will be the main subject of this thesis.
...
After applying the previously mentioned transformations, the number of rows of the dataset is reduced to \emph{87 million}, mainly due to the 8 hours aggregation procedure.
...
By extracting and combining the features using the methods above, a grand total of 52 features were generated. The correlation matrix between these features can be seen in \Cref{fig:correlation}.
...
The initial data exploration showed that the ratio of churn over retained varies in different markets. We, therefore, decided not to limit the samples to users from one market but to collect the data from major markets that are representative of most of the users using the product.
...
Following is the detailed explanation of each experiment and a brief presentation of the results. Further discussion shall be given in \Cref{sec:dis_time_windows}.
...
Both the random forest and logistic regression models exhibit an increase in F1-scores when the size of the observation window is made larger, however the gain can hardly be considered significant. The LSTM model on the other hand show a rather erratic behavior, reaching its top when the window is sized at 49 days. LSTM also exhibits a higher F1-score gain than the other models when the observation window is increased, indicating that a larger accuracy can be achieved if more historical user data is available. Precision-recall curves, ROC curves and a table with all metrics for this experiment can be seen in \Cref{fig:prc_obs_window}, \Cref{fig:roc_obs_window} and \Cref{tab:obs_window}, respectively.
...
The same can not be said about the performance of predictor for the retaining class of \Cref{fig:line_pred_window_ret}. There is a gradual increase of the F1-score as more days are added to the window, however the difference only reflects the changes made in the  distribution of the retaining class. Precision-recall curves, ROC curves and a table with all metrics for this experiment can be seen in \Cref{fig:prc_pred_window}, \Cref{fig:roc_pred_window} and \Cref{tab:pred_window}, respectively.
...
In this evaluation, we assess the performance of the model in terms of both precision and recall given different class distributions in the training data, while keeping the test data with its original skewed distribution of approximately $2.4\%$ of churners. This is accomplished by training several estimators and changing the ratio between the churning and retaining classes. The ratio of churners is initially set to 10\%, with the remaining 90\% corresponding to the retained users. This difference is then gradually reduced until both classes have the same number of samples. We focus on the LSTM and random forest models, since logistic regression underperformed significantly in the previous experiments.

The resulting F1-scores of this experiment for both the churning and retaining classes can be seen in \Cref{fig:line_class_balance} and \Cref{fig:line_class_balance_ret}, respectively. Interestingly the harmonic mean between precision and recall steadily reduces for both classes and models being experimented on. As can be seen in \Cref{tab:class_balance}, increasing the ratio of churning users by undersampling the majority class has an effect of increasing the recall metric, at the cost of a lower precision. If both metrics are equally weighted, the aggregated score suffer a significant decrease for the churning class as the gap between the churning and retaining users is reduced. 
...
This result is contrary to what is exposed by Burez and Van den Poel \citep{Burez2009}. In their study, there is a performance gain for using a less imbalanced dataset for training the predictive models,  however their main evaluation metric is ROC AUC, which could be mapped to our own results in \Cref{fig:roc_class_balance}. 
...
Random forests exhibit a similar behavior, but peaking at 40 principal components instead. The gain however is marginal: approximately $1\%$ of F1-score increase when compared to the full uncompressed dataset, for both retaining and churning users. Extensive metrics can be seen in the precision-recall curves, ROC curves and metrics table at \Cref{fig:prc_dim_reduction}, \Cref{fig:roc_dim_reduction} and \Cref{tab:dim_reduction}, respectively.
